...
import {type Ejemplar, type Libro, type Socio, type Prestamo, Estado, type Rol} from "../Types.ts";
...
export function Catalogo() {
    ...
    //Estas dos reciben el mensaje de no autorizacion desde protected route
    const location = useLocation();
    const mensajeNoAutorizado = location.state?.mensaje;

    const rolActual: Rol = localStorage.getItem("rol") as unknown as Rol;
    const esSocioOAdmin = ["ADMIN", "SOCIO"].includes(rolActual);
                            ...
                            <>
                                <div className="backdrop-blur-sm p-4 rounded-xl flex flex-col gap-3 shadow-xs">
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        ...
                                        <SelectorSocio socios={socios} idSocio={idSocio}
                                                       onChangeSocio={onChangeIdSocio}/>
                                    </div>
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        ...
                                        >
                                            ...
                                            {ejemplarDisponibles.map(e => {
                                                ...
                                                return <option key={e.idEjemplar}
                                                               value={e.idEjemplar}>{libro?.titulo ?? "Libro no encontrado"}</option>;
                                            })}
                                        </select>
                                    </div>
                                ...
                                <div className="backdrop-blur-sm p-4 rounded-xl flex flex-col gap-3 shadow-xs">
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        <label className="text-xs font-bold text-slate-600">Socio:</label>
                                        <SelectorSocio socios={socios} idSocio={idSocioDevolucion}
                                                       onChangeSocio={onChangeIdSocioDevolucion}/>
                                    </div>
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        ...
                                        >
                                            ...
                                            {prestamosActivos.map(p => {
                                                ...
                                                return <option key={p.idEjemplar} value={p.idEjemplar}
                                                               className={"bg-linear-to-r from-amber-300 to-orange-400"}>{p.idEjemplar} - {libroEncontrado?.titulo ?? "libro no encontrado"}</option>;
                                            })}
                                        </select>
                                    </div>
